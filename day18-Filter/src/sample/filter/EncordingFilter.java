 ...
 * 
 * 이 필터가 작동할 주소 형태 등록
 * urlPatterns : /* -> 이 서비스(애플리케이션)의 컨텍스트 패스 이하로 들어오는 모든 요청 주소에 대하여 
 * 
 * @author PC38208
 *
 */
...
public class EncordingFilter implements Filter {
	...
	public void init(FilterConfig config) throws ServletException {
		 ...
		 * init()이 호출될 때 서블릿 컨테이너는 web.xml(@WebFilter)에 선언되어있는
	...
	public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain)
		...
		// 2. 다음필터 호출 : 체인 객체에 다음 필터를 호출, 이 때 요청(req), 응답(res)을 그대로 남김. 이게 핵심.

...
2. request : 응답(response)이 일어나기 전까지 유지되는 객체
			 ...
			 응답이 일어나기 전까지는 해당 request에 설정된
			 attribute가 유지된다.
			 모든 요청(request)은 응답(response)이 발생하면 소멸.
3. session : 명시적으로 생성하는 객체. 명시적으로 제거할 때까지 유지.
			 ...
			 보통은 로그인 정보 등을 유지할 때 소멸.
4. application : 하나의 웹 어플리케이션당 1개씩 생성되는 객체.
				 웹 컨테이너에 하나의 서비스가 배포되어 유지되는 동안
				 계속해서 유지됨.
...
5. 범위를 명시하지 않고 값만 명시하면 어떤 범위가 선택되는가?
	=> 가장 작은 범위의 것이 자동선택.
	\${id} = ${id}
	\${name} = ${name}
</pre>

...
/**
 * session/login.jsp에서 넘어온 요청파라미터
 * (userId, password)로 로그인 성공을 가정하고
 * HttpSession을 사용하여 다음 페이지로 이동처리하는 서블릿.
 */
...
public class LoginServlet extends HttpServlet {
	...
	/**
	 * userId, password 요청 파라미터를 처리하여 로그인 성공을 가정한 후 HttpSession을 사용하여 로그인을 유지.
	 */
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if ("java".equals(userId) && "jsp".equals(password)) {
			 ...
			 * 현재의 요청(request)과 이미 연결된 세션이 존재하면 그 세션을 리턴. 없으면 새로 생성 후 리턴.
			 * 신규 로그인 처리에 사용된다.
			 * 
			 * getSession(false) :
			 * 현재의 요청(request)과 이미 연결된 세션이 존재하면 그 세션을 리턴. 없으면 null 리턴.
			 * 없어도 새로 생성 안한다. 로그아웃 때 사용.
			 * -------------------------------------------------------------------
			 */

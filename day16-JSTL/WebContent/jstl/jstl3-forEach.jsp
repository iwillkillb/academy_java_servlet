...
<h4>&lt;c:forEach&gt;</h4>
<pre>
&lt;c:forEach&gt; : for구문처럼 정해진 횟수 반복, 자료구조 데이터를 자동순회하며 반복
</pre>
...
<c:forEach begin="1" end="10" step="2" varStatus="status" var="result">
	...
	반복의 시작값은? \${status.begin} = ${status.begin}<br/>
	반복의 종료값은? \${status.end} = ${status.end}<br/>
	반복의 증가값은? \${status.step} = ${status.step}<br/>
	<hr>
</c:forEach>

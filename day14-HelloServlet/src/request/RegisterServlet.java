...
public class RegisterServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 1. 요청에 대한 한글처리
				...
				// (2) name이 두 개 이상 동일하게 전달되면 String[]변수로 받는다.
				String[] hobbies = request.getParameterValues("hobby");
				
				// 4. 출력
				// (1) 기본 : sysout 출력
				System.out.println("등록된 사용자 이름 : " + username);
				System.out.println("등록된 사용자 비번 : " + password);
				if (hobbies != null) {
				...
				out.println("등록된 사용자 비번 : " + password + "<br/>");
				// <input> 중에서 type이 checkbox인 경우, 아무것도 선택하지 않은 상황을 고려해야 한다.
				if (hobbies != null) {
					for (String hobby : hobbies) {
